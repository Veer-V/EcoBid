...
const UserDashboard: React.FC<UserDashboardProps> = ({ onLogout }) => {
  ...
  // Data State
  const [user, setUser] = useState(UserService.getUserProfile());
  const [auctions, setAuctions] = useState(UserService.getAuctions());
  const [bids, setBids] = useState(UserService.getBids());
  ...
  const refreshData = () => {
    setUser(UserService.getUserProfile());
    setAuctions(UserService.getAuctions());
    setBids(UserService.getBids());
    setTransactions(UserService.getTransactions());
